# Account-level Cloudflare resources for kwami-app.
#
# Split of ownership (do not manage the same object in both tools):
#   Wrangler  — Worker script, static assets, observability
...
check "hostname_when_custom_domain" {
  assert {
    condition     = !var.enable_custom_domain || (var.app_hostname != "" && var.zone_id != "")
    error_message = "app_hostname and zone_id are required when enable_custom_domain is true."
  }
}
