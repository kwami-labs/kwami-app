...
| --- | --- | --- |
| `VITE_API_URL` | `http://localhost:8080` | [`apiClient.ts`](../../src/lib/apiClient.ts) — token, catalogues, memory, credits, channels, apps |
...
| Connect fails immediately | `VITE_LIVEKIT_URL` empty, `POST /token` down, or CORS |
