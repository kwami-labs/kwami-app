...
watch(
  ...
  (config) => {
    ...
    blob.onClick =
      config.click.enabled && config.click.action !== 'none'
        ? () => executeAction(config.click.action)
        : () => {};

    blob.onDoubleClick =
      config.doubleClick.enabled && config.doubleClick.action !== 'none'
...
<template>
  ...
  <PanelSection :title="t('blobAvatar.material')" icon="ph:sphere-duotone" collapsible>
    ...
    <div class="slider-group">
      <BaseSlider
        ...
        :max="1"
        :step="0.01"
        v-model="skin.opacity"
      />
      <BaseSlider
        :label="t('blobAvatar.shininess')"
        :min="1"
        :max="200"
        :step="1"
        v-model="skin.shininess"
      />
      <BaseSlider
        :label="t('blobAvatar.lightIntensity')"
        :min="0"
        :max="5"
        :step="0.1"
        v-model="skin.lightIntensity"
      />
      <BaseSlider
        :label="t('blobAvatar.resolution')"
        :min="32"
        :max="512"
        :step="8"
        v-model="skin.resolution"
      />
    </div>
  ...
  <PanelSection :title="t('blobAvatar.orientation')" icon="ph:compass-duotone" collapsible>
    <template #actions>
      ...
      >
        <iconify-icon
          :icon="linkPosition ? 'ph:link-duotone' : 'ph:link-break-duotone'"
        ></iconify-icon>
      </button>
      <button
        class="dice-btn"
  ...
  <PanelSection :title="t('blobAvatar.deformation')" icon="ph:asterisk-duotone" collapsible>
    <template #actions>
      ...
      >
        <iconify-icon
          :icon="linkSpikes ? 'ph:link-duotone' : 'ph:link-break-duotone'"
        ></iconify-icon>
      </button>
      <button
        class="dice-btn"
        @click="randomizeSpikes"
        :title="t('blobAvatar.randomizeDeformation')"
      >
        <iconify-icon icon="ph:dice-three-duotone"></iconify-icon>
      </button>
    </template>
    ...
    <div class="slider-group" :class="{ linked: linkSpikes }">
      <BaseSlider
        :label="t('blobAvatar.axisX')"
        :min="0"
        :max="8"
        :step="0.05"
        v-model="shape.spikes.x"
      />
      <BaseSlider
        v-if="!linkSpikes"
        :label="t('blobAvatar.axisY')"
        :min="0"
        :max="8"
        :step="0.05"
        v-model="shape.spikes.y"
      />
      <BaseSlider
        v-if="!linkSpikes"
        :label="t('blobAvatar.axisZ')"
        :min="0"
        :max="8"
        :step="0.05"
        v-model="shape.spikes.z"
      />
    </div>
  </PanelSection>
  ...
  <PanelSection :title="t('blobAvatar.waveAmplitude')" icon="ph:wave-sine-duotone" collapsible>
    <template #actions>
      ...
      >
        <iconify-icon
          :icon="linkAmplitude ? 'ph:link-duotone' : 'ph:link-break-duotone'"
        ></iconify-icon>
      </button>
      <button
        class="dice-btn"
        @click="randomizeAmplitude"
        :title="t('blobAvatar.randomizeAmplitude')"
      >
        <iconify-icon icon="ph:dice-three-duotone"></iconify-icon>
      </button>
    </template>
  ...
  <!-- ==================== AUTO ROTATION ==================== -->
  <PanelSection
    :title="t('blobAvatar.autoRotation')"
  ...
  >
    <template #actions>
      ...
      >
        <iconify-icon
          :icon="linkRotation ? 'ph:link-duotone' : 'ph:link-break-duotone'"
        ></iconify-icon>
      </button>
      <button
        class="dice-btn"
        @click="randomizeRotation"
        :title="t('blobAvatar.randomizeRotation')"
      >
        <iconify-icon icon="ph:dice-three-duotone"></iconify-icon>
      </button>
    </template>
  ...
  <PanelSection :title="t('blobAvatar.idleBreathing')" icon="ph:wind-duotone" collapsible>
    <template #actions>
      <button
        class="dice-btn"
        @click="randomizeBreathing"
        :title="t('blobAvatar.randomizeBreathing')"
      >
        <iconify-icon icon="ph:dice-three-duotone"></iconify-icon>
      </button>
    </template>
  ...
  <PanelSection :title="t('blobAvatar.hoverEffects')" icon="ph:cursor-duotone" collapsible>
    ...
    <div class="interaction-row">
      ...
      <div class="interaction-config" v-if="cursorTouch.hover.enabled">
        <BaseToggle
          :label="t('blobAvatar.highlight')"
          v-model="cursorTouch.hover.highlightOnHover"
        />
        <BaseSelect
          :label="t('blobAvatar.cursor')"
  ...
  <!-- ==================== DRAG INTERACTION ==================== -->
  <PanelSection
    :title="t('blobAvatar.dragInteraction')"
    icon="ph:hand-grabbing-duotone"
    collapsible
  >
  ...
  <PanelSection :title="t('blobAvatar.touchPhysics')" icon="ph:hand-pointing-duotone" collapsible>
    ...
    <div class="slider-group">
      <BaseSlider
        :label="t('blobAvatar.strength')"
        :min="0.1"
        :max="3"
        :step="0.1"
        v-model="cursorTouch.touch.strength"
      />
      <BaseSlider
        :label="t('blobAvatar.durationMs')"
        :min="100"
        :max="3000"
        :step="100"
        v-model="cursorTouch.touch.duration"
      />
      <BaseSlider
        :label="t('blobAvatar.maxPoints')"
        :min="1"
        :max="20"
        :step="1"
        v-model="cursorTouch.touch.maxPoints"
      />
    </div>
  </PanelSection>
  ...
  <PanelSection :title="t('blobAvatar.cursorFollow')" icon="ph:cursor-click-duotone" collapsible>
    <template #actions>
      <button
        class="dice-btn"
        @click="randomizeCursorFollow"
        :title="t('blobAvatar.randomizeCursorFollow')"
      >
        <iconify-icon icon="ph:dice-three-duotone"></iconify-icon>
      </button>
    </template>
    ...
    <div class="slider-group" v-if="cursorTouch.cursorFollow.enabled">
      <BaseSlider
        :label="t('blobAvatar.cursorFollowSensitivity')"
        :min="0.1"
        :max="2.5"
        :step="0.05"
        v-model="cursorTouch.cursorFollow.sensitivity"
      />
    </div>
  </PanelSection>
</template>
