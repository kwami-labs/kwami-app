/**
 * The login screen's record crate, ported from the nexow scroll tour.
 *
 ...
 * login screen has a different soundtrack each time.
 *
 ...
 * the music. A cross-origin file without CORS headers would still play while
 * the analyser silently read zeroes.
 *
 * Nexow's generative `piece-in-d` is deliberately absent. It synthesises itself
 * on its own `AudioContext`, which is not the one the kwami listens to, so it
 * would be the single track the avatar stood still through.
 */
...
export interface Track {
  ...
  readonly src: string;
  /** Where the file was fetched from. Build-time provenance; never used at runtime. */
...
/** Set when the visitor has pressed play once. Namespaced like `kwami.locale`. */
export const FIRST_PLAY_KEY = 'kwami.soundtrack.firstPlay';
 ...
 */
export function pickFirstTrack(
