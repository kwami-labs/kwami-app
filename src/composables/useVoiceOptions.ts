...
export function useVoiceOptions(
  ...
  const sttModels = computed(() => {
    return (
      VOICE_STT_MODELS[stt.value.provider as keyof typeof VOICE_STT_MODELS] || [
  ...
  const llmModels = computed(() => {
    return (
      VOICE_LLM_MODELS[llm.value.provider as keyof typeof VOICE_LLM_MODELS] || [
        { model: 'default', name: 'Default' },
      ]
    );
  });
  ...
  const ttsModels = computed(() => {
    return (
      VOICE_TTS_MODELS[tts.value.provider as keyof typeof VOICE_TTS_MODELS] || [
        { model: 'default', name: 'Default' },
      ]
    );
  });
  ...
  const ttsVoices = computed(() => {
    ...
    return (
      VOICE_FALLBACK_TTS_VOICES[provider as keyof typeof VOICE_FALLBACK_TTS_VOICES] || [
  ...
  const realtimeModels = computed(() => {
    return (
      VOICE_REALTIME_MODELS[realtime.value.provider as keyof typeof VOICE_REALTIME_MODELS] || [
        { model: 'default', name: 'Default' },
      ]
    );
  });
  ...
  const realtimeVoices = computed(() => {
    ...
    // Fall back to hardcoded voices
    return (
      VOICE_FALLBACK_REALTIME_VOICES[provider as keyof typeof VOICE_FALLBACK_REALTIME_VOICES] || [
